			...
			.done(function(response){
				...
				for (var i = response.games.length - 1; i >= 0; i--) {
					if (response.games[i].end_time != null) {
						...
						response.games[i].scores.forEach(function(item){
							...
							$("#table"+i).append("<tr><td>"+actualSlot+"</td><td><span class=\"user"+item.user_id+"\"></span></td><td>"+item.score+"</td><td>"+accuracy+"</td><td>"+a+"</td><td>"+b+"</td><td>"+c+"</td><td>"+d+"</td><td>"+passed+"</td></tr>");
						});
					}
				}
				...
				$(document).ajaxStop(function(){
					...
					Object.keys(results).forEach(function(key,index) {
						...
						$("#accuracyData").append("<tr><td>"+results[key].username+"</td><td>"+best+"</td><td>"+average+"</td><td>"+worst+"</td></tr>");
						$("span.user"+key).empty();
