			...
			.done(function(response){
				...
				for (var i = response.games.length - 1; i >= 0; i--) {
					if (response.games[i].end_time != null) {
						$("#content").append("<div class=\"row\"><h4>Map<b>ID</b>: "+response.games[i].beatmap_id+"</h4></div><div class=\"row\"><table class=\"table table-condensed\" id=\"table"+i+"\"><tr><th>Slot</th><th>Username</th><th>Score</th><th>Accuracy</th><th>300s</th><th>100s</th><th>50s</th><th>Miss</th><th>Status</th><th>URLs</th></tr></table></div>");
						response.games[i].scores.forEach(function(item){
							...
							$("#table"+i).append("<tr><td>"+actualSlot+"</td><td><span class=\"user"+item.user_id+"\"></span></td><td>"+item.score+"</td><td>"+accuracy+"</td><td>"+a+"</td><td>"+b+"</td><td>"+c+"</td><td>"+d+"</td><td>"+passed+"</td><td><a href=\"https://osu.ppy.sh/u/"+item.user_id+"\">Profile</a></td></tr>");
						});
					}
				}
				...
				$(document).ajaxStop(function(){
					...
					document.getElementById("accuracyData").innerHTML = "<tr><th>Username</th><th>Best Accuracy</th><th>Average Accuracy</th><th>Worst Accuracy</th><th>URLs</th></tr>";
					Object.keys(results).forEach(function(key,index) {
						var worst = 100;
						var best = 0;
						var average = 0;
						...
						$("#accuracyData").append("<tr><td>"+results[key].username+"</td><td>"+best+"</td><td>"+average+"</td><td>"+worst+"</td><td><a href=\"https://osu.ppy.sh/u/"+key+"\">Profile</a></td></tr>");
						$("span.user"+key).empty();
